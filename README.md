...
![Version](https://img.shields.io/badge/version-v2.2.0-2563eb?style=flat-square)
...
- [下载 Topdo v2.2.0 macOS Universal DMG](https://github.com/SkyNone/Topdo/releases/download/v2.2.0/Topdo_2.2.0_universal.dmg)
- [查看 v2.2.0 Release](https://github.com/SkyNone/Topdo/releases/tag/v2.2.0)
...
- **日有所成**：汇总当天完成的任务与习惯，支持保存为图片、复制文字，并根据完成数量展示不同激励文案。
- **飞书字段映射**：标签可绑定已有多选字段；多个飞书枚举值可映射到同一个 Topdo 优先级，适配不同团队的表格结构。
- **重复任务可靠性**：停止重复时会同时停用原始任务和全部周期实例，避免后续编辑后重复规则再次生效。
- **导出反馈优化**：“日有所成”保存后展示完整路径，可直接在 Finder 中查看；复制和保存反馈始终显示在可见区域。
- **同步与数据体验修复**：继续改善飞书标签同步、历史完成统计和本地缓存一致性。
...
[下载 `Topdo_2.2.0_universal.dmg`](https://github.com/SkyNone/Topdo/releases/download/v2.2.0/Topdo_2.2.0_universal.dmg)，拖拽 `Topdo.app` 到 `Applications`。
...
- `.dmg`：`src-tauri/target/universal-apple-darwin/release/bundle/dmg/Topdo_2.2.0_universal.dmg`
