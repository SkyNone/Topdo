...
![Version](https://img.shields.io/badge/version-v2.1.2-2563eb?style=flat-square)
![Arch](https://img.shields.io/badge/arch-Universal%20(Apple%20Silicon%20%2B%20Intel)-10b981?style=flat-square)

![Topdo 产品介绍图](docs/images/topdo-v2.0.2-promo.jpg)
...
- [下载 Topdo v2.1.2 macOS Universal DMG](https://github.com/SkyNone/Topdo/releases/download/v2.1.2/Topdo_2.1.2_universal.dmg)
- [查看 v2.1.2 Release](https://github.com/SkyNone/Topdo/releases/tag/v2.1.2)
...
| 飞书同步 | 对接飞书多维表格，支持任务字段、标签同步与优先级映射 |
| 任务三态 | 待办 / 进行中 / 已完成，适合执行推进 |
| 优先级分组 | 紧急 / 重要 / 普通分组展示，紧急固定展开，重要和普通可收起 |
| 搜索过滤 | `⌘F` 按任务名称和备注搜索，顶部保留搜索状态 |
| 自定义标签 | 每个任务最多 5 个标签，保留最近 5 个标签便于复用 |
...
- **自定义标签**：任务支持轻量标签，最近 5 个标签可复用；标签会同步到飞书多维表格。
- **同优先级内排序**：支持在同一优先级分组内手动调整任务顺序，适合整理当天执行顺序。
- **分组收起优化**：`重要 / 普通` 支持收起，`紧急` 固定展开，避免关键任务被隐藏。
- **新建任务防误触**：新建面板内改为 `⌘Enter` 创建任务，普通回车不再误创建，中文输入更安全。
- **飞书同步可靠性**：修复重复创建、长期待同步、缺失字段、标签未同步等问题；旧模板缺少 `标签` 字段时会尝试自动创建。
- **循环任务更稳定**：按“原始任务 + 到期日期 + 循环规则”去重，避免同一期循环任务被重复生成。
- **完成反馈更轻量**：从进行中切到已完成时会有克制的小彩蛋动效，清空任务时给出轻提示。
- **迷你模式更可控**：长按拖动迷你胶囊时不再误展开主窗口，短按恢复和拖动移动区分更清楚。
- **体验细节修复**：暗色模式、新建/详情页输入、快捷键说明、标签展示、顶部创建按钮换行等细节继续打磨。
...
[下载 `Topdo_2.1.2_universal.dmg`](https://github.com/SkyNone/Topdo/releases/download/v2.1.2/Topdo_2.1.2_universal.dmg)，拖拽 `Topdo.app` 到 `Applications`。
...
如果模板里没有 `标签` 字段，Topdo 会在同步标签时尝试自动创建文本字段 `标签`；如果应用权限不足，也可以在多维表格里手动新增该字段。
...
- `.dmg`：`src-tauri/target/universal-apple-darwin/release/bundle/dmg/Topdo_2.1.2_universal.dmg`
